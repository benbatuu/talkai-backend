...
import { env } from 'bun';
import { generateRefreshToken } from '../../helpers/functions';
import UserStatus from '../../enums/userStatuses';
import HttpStatusCode from '../../enums/httpstatuscode';
import * as bcrypt from 'bcrypt';

export const loginService = async (apiclientid: string, email: string | undefined, password: string | undefined, refreshtoken: string | undefined) => {
    ...
    const user = await pc.user.findFirst({
        ...
        where: { ...(refreshtoken ? { id: existingRefreshToken.userid } : { email: email }) },
    });
    ...
    if (!refreshtoken && password) {
        ...
        if (!passwordMatch) {
            throw new HTTPException(400, { message: 'invalid_email_or_password' });
        }
    }
